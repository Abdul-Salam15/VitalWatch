...
export function CaregiverMeds({ reminders, now }: CaregiverMedsProps) {
  const active = reminders.filter((r) => r.active);
  return (
    <Card className="p-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <SectionTitle icon="pill" title="Medications" sub={`${active.length} active · schedule, intake & status`} />
        <span className="text-sm font-semibold text-slate-500">Overall adherence <span className="text-brand font-extrabold">{weekAdherencePct(reminders, now)}%</span></span>
      </div>
      {reminders.length === 0 ? (
        ...
        <div className="mt-4 grid grid-cols-1 xl:grid-cols-2 gap-3">
          {reminders.map((r) => <CaregiverMedCard key={r.id} r={r} now={now} />)}
        </div>
      )}
